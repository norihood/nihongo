...
class Wp_Custom_Register_Login_Public extends Wp_Custom_Register_Login_Generic_Public
{
     ...
     * @since   1.0.0
     */
    public function wpcrl_display_change_pass_form()
    {
        include_once 'partials/wpcrl-change-pass-form.php';
    }
     ...
     * @since   1.0.0
     */
    public function wpcrl_user_change_pass()
    {
        // $wpcrl_email_settings = get_option('wpcrl_email_settings');
        // $wpcrl_messages_settings = get_option('wpcrl_display_settings');
        $response = array();
        ...
        if (isset($_POST) && $_POST) {
            ...
            $credentials = array();
            // $credentials['user_login'] = trim($_POST['wpcrl_username']);
            $credentials['user_password'] = trim($_POST['wpcrl_password_old']);
            $credentials['user_password_new'] = trim($_POST['wpcrl_password_new']);

            $user = get_currentuserinfo();
            if ( $user && wp_check_password( $credentials['user_password'], $user->data->user_pass, $user->ID) ) {

                $user_id = wp_update_user( array( 'ID' => $user->ID, 'user_pass' => $credentials['user_password_new'] ) );

                if ( is_wp_error( $user_id ) ) {
                    $response['error'] = __('Update fail', $this->plugin_name);
                } else {
                    $response['status'] = true;
                    $response['success'] = __('Password is changed successfully!', $this->plugin_name);
                }

            } else {
                $response['error'] = __('Password is wrong.', $this->plugin_name);
            }
