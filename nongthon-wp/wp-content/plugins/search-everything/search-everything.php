...
class SearchEverything {
    ...
    function search_hooks() {
        ...
        // add custom post type - PhuongNLT
        add_filter('posts_where', array(&$this, 'se_page_gioi_thieu'));
        ...
        add_filter('posts_where', array(&$this, 'se_only_post_type'));

        add_filter('posts_request', array(&$this, 'se_log_query'), 10, 2);
    }

    function se_page_gioi_thieu($where) {
        global $wpdb;
        $regex = '/(' . $wpdb->posts . '\.post_type IN \([^\)].*?\))/';
        $where = preg_replace($regex, '(${1} OR (' . $wpdb->posts . '.post_type = \'page\' AND ' . $wpdb->posts . '.post_name = \'gioi-thieu\'))', $where);
//        echo '<pre>123';
//        print_r($where);
//        die();
        return $where;
    ...
    // only post type - PhuongNLT
    function se_only_post_type($where) {
        global $wpdb;
        if (!empty($_GET['m'])) {
            $search_in = $_GET['m'];
            switch ($search_in) {
                case 'he-thong-to-chuc':
                    $where = 'AND (' . substr($where, strpos($where, 'AND') + 3) . ') AND (tter.slug IN (\'he-thong-to-chuc\', \'cap-tinh\', \'cap-huyen\'))';
                    break;
                case 'gioi-thieu':
                    $where = 'AND (' . substr($where, strpos($where, 'AND') + 3) . ') AND post_name = \'gioi-thieu\'';
                    break;
                case 'van-ban':
                    $where = 'AND (' . substr($where, strpos($where, 'AND') + 3) . ') AND post_type = \'laws\'';
                    break;
                case 'hoi-dap':
                    $where = 'AND (' . substr($where, strpos($where, 'AND') + 3) . ') AND post_type = \'faq\'';
                    break;
                case 'tin-tuc':
                    $where = 'AND (' . substr($where, strpos($where, 'AND') + 3) . ') AND post_type = \'post\' AND (tter.slug NOT IN (\'he-thong-to-chuc\', \'cap-tinh\', \'cap-huyen\'))';
                    break;
                default:
                    break;
            }
