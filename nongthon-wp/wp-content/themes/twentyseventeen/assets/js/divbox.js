        ...
        function _run(index, init) {
            ...
            fn.closed = function () {
                $('#divbox_frame').animate({
                    ...
                    if (typeof (_cfg.api.closed) == 'function')
                        _cfg.api.closed(this);
                });
            };
            fn.toggleObj = function (o, act) {
                if (ie6) {
                    if (act == 'show')
                        $(o).find('embed,object,select').show();
                    else
                        $(o).find('embed,object,select').hide();
                }
            };
            fn.init = function () {
                if (typeof (_cfg.api.start) == 'function')
                    _cfg.api.start(obj);
            ...
            fn.main = function (t, l, w, h, fncallback, fnclosed, caption) {
                ...
                if (typeof (_cfg.api.beginLoad) == 'function')
                    _cfg.api.beginLoad(oContent);
                if (typeof (fncallback) == 'function')
                    fncallback(oContent);
                if (typeof (_cfg.api.afterLoad) == 'function')
                    _cfg.api.afterLoad(oContent);
            ...
            fn.flashEmbedString = function (file, w, h, type) { // default type is FLV
                ...
                if (type != 'swf')
                    str += '<param name="flashvars" value="file=' + file + '&autostart=true' +
                            flashvar + '" />';
                str +=
